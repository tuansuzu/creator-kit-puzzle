...
public class SceneMenu : MonoBehaviour
{
    ...
    void CreateGrid ()
    {
        ...
        List<LevelInfo> nonignoredLevels = new List<LevelInfo> ();

        for (int i = 0; i < levels.Count; i++)
        {
            if(!levels[i].level.ignore)
                nonignoredLevels.Add (levels[i]);
        }
        
        if(nonignoredLevels.Count <= 0)
            throw new UnityException("All Scene References assigned to construct a menu from are being ignored.  Check the Scene References that have been assigned to stop them from being ignored.");
        ...
        float sqrt = Mathf.Sqrt (nonignoredLevels.Count);
        int gridX = Mathf.CeilToInt (sqrt);
        int gridY = gridX * (gridX - 1) > nonignoredLevels.Count ? gridX - 1 : gridX;
        ...
        for (int i = 0; i < gridY; i++)
        {
            for (int j = 0; j < gridX; j++)
            {
                ...
                if(sceneIndex > nonignoredLevels.Count - 1)
                    return;
                
                SceneMenuIcon sceneMenuIcon = Instantiate (sceneMenuIconPrefab, parentRectTransform);

                sceneMenuIcon.sceneReference = nonignoredLevels[sceneIndex].level;
                sceneMenuIcon.sceneMenu = this;
                sceneMenuIcon.textMesh.text = nonignoredLevels[sceneIndex].displayName;
